...
def _track(inspector_id: str, z_values: list[float]) -> AFZTrack:
    samples = [
...
def _scan_result(z_values: list[float], focus_measure: float) -> ScanResult:
    return ScanResult(
        af_z_map=AFZMap(tracks={"INS1": _track("INS1", z_values)}),
        scan_images=ScanImageSet(focus_measures={"INS1": focus_measure}),
    )
...
class OpticalSubsystemCheckTest(unittest.TestCase):
    ...
    def test_check_item_name_is_per_inspector(self) -> None:
        self.assertEqual(check_item_name("INS1"), "INS1 광학계 상태 확인")
    ...
    def test_all_sub_judgements_pass(self) -> None:
        ...
        self.focus_store.save_criteria(check_item_name("INS1"), "focus_ratio", 0.85, 1.15)
    ...
    def test_tilt_failure_dominates_even_if_focus_passes(self) -> None:
        self.dof_store.save_criteria(DOF_CHECK_ITEM, "INS1", 0.0, 1.0)
        run_optical_subsystem_check(
            "INS1",
            _scan_result([50.0, 50.0, 50.0], 1000.0),
            self.dof_store,
