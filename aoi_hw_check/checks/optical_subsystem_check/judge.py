...
def check_item_name(subsystem: str) -> str:
    """계통(Micro/Macro/계측)별 광학계 상태 확인 항목명 — Focus와 Tilt를 함께 판정한다."""
    return f"{subsystem} 광학계 상태 확인"


def _evaluate_tilt(
    subsystem: str, scan_result: ScanResult, dof_criteria_store: CriteriaStore
) -> tuple[Verdict, list[FieldMismatch], str]:
    """AF Z 맵에 평면을 피팅해 기울기 크기·방향을 산출하고, Tilt로 인한 초점
    이탈(측정 영역 내 높이 변화량)이 광학계 DOF 이내인지 판정한다."""
    track = scan_result.af_z_map.tracks.get(subsystem)
...
def _evaluate_focus(
    ...
    절대값이 아닌 상대 비교이므로, 이 설비/계통의 최초 실행 결과를 기준
    시료 값으로 저장하고(baseline), 이후 실행은 baseline 대비 비율을
    CriteriaStore의 허용 범위와 비교한다.
    """
    value = scan_result.scan_images.focus_measures.get(subsystem)
    if value is None:
...
def run_optical_subsystem_check(
    ...
    """광학계 1계통(Micro/Macro/계측)의 Focus·Tilt를 함께 판정해 하나의 결과로 합친다."""
    check_item = check_item_name(subsystem)

    tilt_verdict, tilt_deviation, tilt_note = _evaluate_tilt(
        subsystem, scan_result, dof_criteria_store
    )
    focus_verdict, focus_deviation, focus_note = _evaluate_focus(
        subsystem, scan_result, focus_criteria_store, store, equipment_id, check_item
    )
