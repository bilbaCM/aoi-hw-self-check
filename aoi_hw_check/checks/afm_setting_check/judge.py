...
def run_afm_setting_check(
    ...
    for inspector_id, track in scan_result.af_z_map.tracks.items():
        ...
        if dof is None:
            notes.append(f"{inspector_id}: 추종 오차 {max_error:.4f}um — DOF 미등록")
            continue

        evaluated += 1
        allowed = dof.max_value * _DOF_FRACTION
        notes.append(f"{inspector_id}: 추종 오차 {max_error:.4f}um / 허용 {allowed:.4f}um (DOF/3)")
        if max_error > allowed:
            deviations.append(
                FieldMismatch(
                    field_path=inspector_id,
    ...
    if evaluated == 0:
        ...
        detail = f"추종 오차 초과 {len(deviations)}/{evaluated}대 — " + "; ".join(notes)
    else:
        verdict = Verdict.PASS
        detail = f"전 {evaluated}대 추종 오차 기준 이내 — " + "; ".join(notes)
